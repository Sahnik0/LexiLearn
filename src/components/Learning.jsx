...
function Learning() {
  ...
  const calculateAndSaveScore = async () => {
    const correctAnswers = Object.entries(userAnswers).filter(
      ([index, answer]) => answer === modules[activeModule].quiz[index].correct
    ).length;
    
    const score = (correctAnswers / modules[activeModule].quiz.length) * 100;
    setQuizScore(score);
    stopTimer();

    await updateProgress(modules[activeModule].id, {
      completed: true,
      score,
      timeSpent: time,
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl max-w-lg w-full p-6">
              <div className="text-center">
                ...
                <p className="mb-6">Time taken: {formatTime(time)}</p>
